...
$(".content_sidebar-left").on("mousedown", "button.item.btn,button.menu_item", function(evt) {
  ...
  if($(this).attr("data-qa") == "Signature") {
    ...
    newTag = $('<div class="newTagDraggable draggable DataTabView" data-name="' + $(this).attr("data-qa") + '" data-tag-for="' + tagForRecipient + '"><div style="display: flex"><div class="the_tag" style="background-color: ' + svgFillColor + '"><textarea readonly class="input-textarea" rows="1" placeholder="Text"></textarea></div></div></div>');
    // $("#documentTags").append(newTag);
    $(".tagger_content").append(newTag);
    moveNewTag(evt);
...
function showOptions(element, isShiftPressed) {
  ...
  // selectedTag.find(".the_tag textarea, .the_tag input, .the_tag select").first().focus();
...
$("body").on("keydown", function(evt) {
  ...
  if(evt.keyCode == 27) {
    ...
    if($(".PaymentTabView").hasClass("selected")) {
      selectedTag.find(".the_tag input").val(lastPaymentAmount);
    }
  ...
  if((evt.keyCode == 32) && $(".DropdownTabView").hasClass("selected")) {
    evt.preventDefault();
  ...
  if((evt.keyCode == 13) && $(".DropdownTabView").hasClass("selected")) {
    ...
    if(!selectedTag.find(".dropdown_menu .select-menu").hasClass("invisible")) {
      ...
      selectedTag.toggleClass("isEditing");
    }
  }
  ...
  if(selectedTag && ((evt.keyCode >= 48 && evt.keyCode <= 57) || (evt.keyCode >= 65 && evt.keyCode <= 90))) {
    // evt.preventDefault();
    if(!selectedTag.find(".the_tag input, .the_tag textarea").is(":focus")) {
      // selectedTag.find(".the_tag input, .the_tag textarea").focus();
    }
  }
  ...
  if(evt.keyCode == 37 && selectedTag && !selectedTag.find(".the_tag textarea").is(":focus")) {
    evt.preventDefault();
    if(parseInt(selectedTag.css("left")) > 0) {
      selectedTag.css("left", parseInt(selectedTag.css("left")) - 15);
    }
  }
  if(evt.keyCode == 38 && selectedTag && !selectedTag.find(".the_tag textarea").is(":focus")) {
    evt.preventDefault();
    if(!selectedTag.hasClass("isEditing")) {
      if(parseInt(selectedTag.css("top")) > 0) {
        selectedTag.css("top", parseInt(selectedTag.css("top")) - 15);
      }
      ...
      if(selectedTag.find(".dropdown_menu .menu .item.on").prev().length > 0 && selectedTag.find(".dropdown_menu .menu").hasClass("invisible")) {
        selectedTag.find(".dropdown_menu .menu .item.on").prev(".item").click();
      }
    }
  }
  if(evt.keyCode == 39 && selectedTag && !selectedTag.find(".the_tag textarea").is(":focus")) {
    evt.preventDefault();
    if(selectedTag.closest(".PageView").innerWidth() - (parseInt(selectedTag.css("left")) + parseInt(selectedTag.css("width"))) > 2) {
      selectedTag.css("left", parseInt(selectedTag.css("left")) + 15);
    }
  }
  if(evt.keyCode == 40 && selectedTag && !selectedTag.find(".the_tag textarea").is(":focus")) {
    evt.preventDefault();
    ...
    if(!selectedTag.hasClass("isEditing")) {
      ...
      if(selectedTag.find(".dropdown_menu .menu .item.on").next().length > 0 && selectedTag.find(".dropdown_menu .menu").hasClass("invisible")) {
        selectedTag.find(".dropdown_menu .menu .item.on").next(".item").click();
      }
    }
  }
  if((evt.keyCode == 13) && !evt.shiftKey && selectedTag && selectedTag.find(".the_tag textarea").is(":focus")) {
    evt.preventDefault();
  ...
  } else if((evt.keyCode == 13) && !evt.shiftKey && selectedTag && !selectedTag.find(".the_tag textarea").is(":focus")) {
    evt.preventDefault();
...
$("body").on("focus", ".PaymentTabView.newTagDraggable.selected .the_tag input", function(evt) {
...
$(".content_main").on("focus", ".newTagDraggable .the_tag input[type='text'], .newTagDraggable .the_tag textarea", function(evt) {
  // evt.stopPropagation();
  deleteTagAllowed = false;
});
$(".content_main").on("blur", ".newTagDraggable .the_tag input[type='text'], .newTagDraggable .the_tag textarea", function(evt) {
