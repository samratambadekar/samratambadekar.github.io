$(document).ready(function() {
  ...
  window.setTimeout(function() {
    ...
    //   disablePan: true
    // }
    // $(".disclosure_page > img").panzoom();
    // $(".document_image").panzoom();
    // $(".disclosure_page > img").panzoom({disablePan: true, minScale: 1});
    // $(".document_page, .disclosure_page > img").panzoom({
    ...
    // FOR 2020 DEMO, FOR JOE
    $("#disclosureAccepted").click();
  ...
  $("#action-bar-btn-continue").on("click", function() {
    ...
    if(!all_completed) {
      ...
      $("#action-bar-btn-continue > .desktop").text("Next");
  ...
  $("#field-2, #field-4").on("click", function() {
    ...
    if(isAdoptedSignature) {
      if(!$(this).find("img:first-child").hasClass("hide")) {
        ...
        $("#action-bar-btn-continue > .desktop").text("Pay Now");
        $("#action-bar-btn-continue").addClass("btn_pay_amount");
      } else {
        ...
        $("#action-bar-btn-continue > .desktop").text("Next");
        $("#action-bar-btn-continue").removeClass("btn_pay_amount");
      }
      ...
      signTagTarget = $(this);
      $("#adoptSignatureModal").removeClass("hidden");
    }
  });
  ...
  $("#adoptSignatureModal").on("click", ".modal_footer button", function() {
    $(this).closest(".dialog-holder").addClass("hidden");
  });
  $("#payAmountModal").on("click", ".header", function() {
    $(this).closest(".dialog-holder").addClass("hidden");
  });
  $("#adoptSignatureModal").on("click", "button[data-qa='adopt-submit']", function() {
    isAdoptedSignature = true;
    signTagTarget.click();
  });

  $("body").on("click", ".btn_pay_amount", function() {
    $("#payAmountModal").removeClass("hidden");
  });

  $("body").on("mouseenter", ".button-icon", function() {
    $(".cvv_hover").removeClass("hide");
  });
  $("body").on("mouseleave", ".button-icon", function() {
    $(".cvv_hover").addClass("hide");
  });

  $("#payAmountModal").on("click", ".btn[data-qa='payment-submit']", function() {

    var element = $(this);

    if(!$(".payment_step_3").hasClass("hide")) {
      // $("#payAmountModal").addClass("hide");
      // $("#signUpModal").removeClass("hide");

      window.location.href = "documents1.html";
    }

    if(!$(".payment_step_1").hasClass("hide")) {
      $("#payAmountModal .payment_step_1").addClass("hide");
      $("#payAmountModal .payment_step_2").removeClass("hide");
      element.addClass("disabled");

      $("#payAmountModal .header").css("pointer-events", "none");
      $("#payAmountModal button.x-close").addClass("hide");

      setTimeout(function() {
        $("#payAmountModal .payment_step_2").addClass("hide");
        $("#payAmountModal .payment_step_3").removeClass("hide");
        element.removeClass("disabled").text("Finish");
      }, 2000);
    }
  });
});
