...
$(".content_sidebar-left").on("mousedown", "button.item.btn,button.menu_item", function(evt) {
  ...
  if($(this).attr("data-qa") == "Signature") {
    ...
    newTag = $('<div class="newTagDraggable draggable RadioTabView" data-name="' + $(this).attr("data-qa") + '" data-tag-for="' + tagForRecipient + '"><div style="display: flex" class="radio_sub"><div class="the_tag" style="background-color: ' + svgFillColor + '"><input type="radio" class="input-rb" name="radio_grp_' + radio_grp_num + '"/></div></div><div style="display: flex" class="radio_sub"><div class="the_tag" style="background-color: ' + svgFillColor + '"><input type="radio" class="input-rb" name="radio_grp_' + radio_grp_num + '"/></div></div><div style="display: flex" class="radio_sub"><div class="the_tag" style="background-color: ' + svgFillColor + '"><input type="radio" class="input-rb" name="radio_grp_' + radio_grp_num + '"/></div></div><div style="display: flex" class="radio_sub"><div class="the_tag" style="background-color: ' + svgFillColor + '"><input type="radio" class="input-rb" name="radio_grp_' + radio_grp_num + '"/></div></div><div style="display: flex" class="radio_sub"><div class="the_tag" style="background-color: ' + svgFillColor + '"><input type="radio" class="input-rb" name="radio_grp_' + radio_grp_num + '"/></div></div></div>');
    // $("#documentTags").append(newTag);
    $(".tagger_content").append(newTag);
    moveNewTag(evt);
    ...
    // } else {
      // addPaymentGateway();
    // }
  } else if($(this).attr("data-qa") == "Notarize") {
    newTag = $('<div class="newTagDraggable draggable NotarizeTabView" data-name="' + $(this).attr("data-qa") + '" data-tag-for="' + tagForRecipient + '"><div style="display: flex; width: 100%"><div class="the_tag" style="width: 100px; height: 25px; text-align: center; display: flex; justify-content: center; align-items: center; background-color: ' + svgFillColor + '"><span class="icon icon-notary"></span> <div class="tag_text" style="margin-left: 5px;">Notarize</div></div></div></div>');
...
function dragMoveListener (event) {
  ...
  if(!$(event.target).find("textarea").is(':focus') && !$(event.target).hasClass("NotarizeTabView")) {
...
function showOptions(element, isShiftPressed) {
  ...
  if(selectedTag.hasClass("RadioTabView")) {
    if(isShiftPressed) {
      selectedTag.children(".radio_sub").removeClass("radio_selected").last().addClass("radio_selected");
    } else {
      selectedTag.children(".radio_sub").removeClass("radio_selected").first().addClass("radio_selected");
    }
  }
  ...
  $(".drawer-properties").find(".drawer-properties_header .icon").removeClass().addClass("icon icon-palette-field-" + element.attr("data-name").toLowerCase() + "");

  var sidebar_tag_title_text = "";

  if(element.attr("data-name")=="Disclosure") {
    sidebar_tag_title_text = "Supplement";
  } else if (element.attr("data-name")=="Notarize") {
    sidebar_tag_title_text = "Notary";
  } else {
    sidebar_tag_title_text = element.attr("data-name");
  }

  $(".drawer-properties").find(".drawer-properties_header .ng-binding").text(sidebar_tag_title_text);
  $(".content_sidebar-right").css({"overflow": "hidden"});
  // console.log($(".content_sidebar-right .drawer-properties_main .toolbarRecipientsMenu .item[data-recipient-name='" + selectedTag.attr("data-tag-for") + "']"));
  $(".content_sidebar-right .drawer-properties_main .toolbarRecipientsMenu .item[data-recipient-name='" + selectedTag.attr("data-tag-for") + "']").click();

    if(selectedTag.hasClass("NotarizeTabView")) {
      $(".content_sidebar-right .toolbarRecipientsMenu [data-can-notarize]").addClass("hidden");
      $(".content_sidebar-right .toolbarRecipientsMenu [data-can-notarize='true']").removeClass("hidden");
    } else if(!selectedTag.hasClass("NotarizeTabView")) {
      $(".content_sidebar-right .toolbarRecipientsMenu [data-can-notarize]").addClass("hidden");
      $(".content_sidebar-right .toolbarRecipientsMenu [data-can-notarize='false']").removeClass("hidden");
    }

  // $(".drawer-properties .toolbarRecipientsMenu .item[data-recipient-name='" + selectedTag.attr("data-tag-for") + "']").addClass("hidden");
  ...
  if(element.attr("data-name")=="Signature") {
    $(".drawer-properties").find(".drawer-properties_header .icon").removeClass().addClass("icon icon-palette-field-sign");
  ...
  if(element.attr("data-name")=="Notarize") {
    $(".drawer-properties").find(".drawer-properties_header .icon").removeClass().addClass("icon icon-notary");
  }
}
...
$("body").on("keydown", function(evt) {
  ...
  if(evt.keyCode == 9 && selectedTag && !$(".RadioTabView").hasClass("selected")) {
    evt.preventDefault();
    if(evt.shiftKey) {
      tabToPrevTag();
    } else {
    ...
    evt.preventDefault();
    if(!evt.shiftKey && selectedTag.find(".radio_sub.radio_selected").next(".radio_sub").length > 0) {
      selectedTag.find(".radio_sub.radio_selected").removeClass("radio_selected").next(".radio_sub").addClass("radio_selected");
    } else if(evt.shiftKey && selectedTag.find(".radio_sub.radio_selected").prev(".radio_sub").length > 0) {
      selectedTag.find(".radio_sub.radio_selected").removeClass("radio_selected").prev(".radio_sub").addClass("radio_selected");
    } else {
      if(evt.shiftKey) {
        tabToPrevTag();
      } else {
  ...
  if(evt.keyCode == 40 && $(".RadioTabView").hasClass("selected")) {
    evt.preventDefault();
    if(selectedTag.find(".radio_sub.radio_selected").next(".radio_sub").length > 0) {
      selectedTag.find(".radio_sub.radio_selected").removeClass("radio_selected").next(".radio_sub").addClass("radio_selected");
    }
  }
  if(evt.keyCode == 38 && $(".RadioTabView").hasClass("selected")) {
    evt.preventDefault();
    if(selectedTag.find(".radio_sub.radio_selected").prev(".radio_sub").length > 0) {
      selectedTag.find(".radio_sub.radio_selected").removeClass("radio_selected").prev(".radio_sub").addClass("radio_selected");
    }
  }
  ...
  if((evt.keyCode == 13 || evt.keyCode == 32) && $(".RadioTabView").hasClass("selected")) {
    evt.preventDefault();
    selectedTag.find(".radio_sub.radio_selected input").prop("checked", true);
  }
  ...
  if(evt.keyCode == 37 && selectedTag && !selectedTag.hasClass("NotarizeTabView") && !selectedTag.find(".the_tag textarea, .the_tag input[type='number']").is(":focus")) {
    evt.preventDefault();
  ...
  if(evt.keyCode == 38 && selectedTag && !selectedTag.hasClass("NotarizeTabView") && !selectedTag.find(".the_tag textarea, .the_tag input[type='number']").is(":focus")) {
    evt.preventDefault();
    if(!selectedTag.hasClass("isEditing")) {
  ...
  if(evt.keyCode == 39 && selectedTag && !selectedTag.hasClass("NotarizeTabView") && !selectedTag.find(".the_tag textarea, .the_tag input[type='number']").is(":focus")) {
    evt.preventDefault();
  ...
  if(evt.keyCode == 40 && selectedTag && !selectedTag.hasClass("NotarizeTabView") && !selectedTag.find(".the_tag textarea, .the_tag input[type='number']").is(":focus")) {
    evt.preventDefault();
  ...
  if(evt.keyCode == 8 && $(".draggable").hasClass("selected") && deleteTagAllowed) {
    evt.preventDefault();
    deleteTag(selectedTag);
  }
});
...
function deleteTag(tagElem) {
  ...
  if(tagElem.hasClass("NotarizeTabView")) {
    // tagElem.closest(".PageView").css({"padding-top": parseInt(tagElem.closest(".PageView").css("padding-top")) - 25});
    tagElem.nextAll(".NotarizeTabView").each(function() {
...
$(".toolbar_tagger").on("click", ".toolbarRecipientsMenu button.item", function(e) {
  ...
  target.find(".ng-binding").text($(this).find(".ng-binding.notary_dropdown_recipient .notary_recipient_name").text());
  ...
  $("#sidebarLeft .sidebarTabs_tab-on .icon").css("color", svgFillColor);

  checkCanNotarize($(this).closest("li"));
});

$(".content_main, .content_sidebar-right").on("click", ".toolbarRecipientsMenu button.item", function(evt) {
  if(selectedTag.hasClass("NotarizeTabView") && !$(this).closest("li").attr("data-can-notarize")) {
    showToastMessageOlive($(".msg_notary_added"), "User cannot be marked for notarization");
    return;
  } else if(!selectedTag.hasClass("NotarizeTabView") && $(this).closest("li").attr("data-can-notarize") == true) {
    showToastMessageOlive($(".msg_notary_added"), "User is marked for notarization");
    return;
  }
  var target = $(this).closest(".toolbarRecipientsMenu").prev("button.btn-recipients");
  $(this).closest(".toolbarRecipientsMenu").find("button.item").removeClass("on");
  ...
  if(target.find(".swatch-recipient").hasClass("swatch-ext-0")) {
    ...
    selectedTagFillColor = "#acdce6";
  } else if(target.find(".swatch-recipient").hasClass("swatch-ext-2")) {
    selectedTagFillColor = "#c0a5cf";
  } else if(target.find(".swatch-recipient").hasClass("swatch-ext-3")) {
    selectedTagFillColor = "#97c9bf";
  } else if(target.find(".swatch-recipient").hasClass("swatch-ext-4")) {
    selectedTagFillColor = "#f7b994";
  } else if(target.find(".swatch-recipient").hasClass("swatch-ext-5")) {
    selectedTagFillColor = "#c3d5e6";
  } else if(target.find(".swatch-recipient").hasClass("swatch-ext-6")) {
    selectedTagFillColor = "#cfdb7f";
  } else if(target.find(".swatch-recipient").hasClass("swatch-ext-7")) {
    selectedTagFillColor = "#ff9980";
  } else if(target.find(".swatch-recipient").hasClass("swatch-ext-8")) {
    selectedTagFillColor = "#e6c6e6";
  } else if(target.find(".swatch-recipient").hasClass("swatch-ext-9")) {
    selectedTagFillColor = "#ffb3c6";
  } else {
    selectedTagFillColor = "#ffd65b";
  }

  if($(this).closest(".content_sidebar-right").length > 0) {
    selectedTag.attr("data-tag-for", $(this).find(".ng-binding .notary_recipient_name").text());
    selectedTag.find(".the_tag").css("background-color", selectedTagFillColor);
  } else {
    $(this).closest(".newTagDraggable").attr("data-tag-for", $(this).find(".ng-binding .notary_recipient_name").text());
...
function checkCanNotarize(selectedUser) {
  if(selectedUser.attr("data-can-notarize") == "true") {
    $("#sidebarLeft").addClass("notary_recipient");
    $("#sidebarLeft .sidebarTabs_tab[data-qa='Notary Fields']").click();

    $("#sidebarLeft .sidebar-fields .menu_listItem").removeClass("show_mini_tooltip");
    $("#sidebarLeft .sidebar-fields[data-tab-name='Standard Fields'] .menu_listItem").addClass("show_mini_tooltip");
  } else {
    $("#sidebarLeft").removeClass("notary_recipient");
    $("#sidebarLeft .sidebarTabs_tab[data-qa='Standard Fields']").click();

    $("#sidebarLeft .sidebar-fields .menu_listItem").removeClass("show_mini_tooltip");
    $("#sidebarLeft .sidebar-fields[data-tab-name='Notary Fields'] .menu_listItem").addClass("show_mini_tooltip");
  }
}
...
$(".content_main").on("mouseup", ".PageView", function(evt) {
  ...
  $(this).find(".pageHighlightOverlay").css({"background-color": "transparent", "border-color": "transparent"});
  attachTagItem($(this), evt.pageX - $(this).offset().left - tagElem.width()/2, evt.pageY - $(this).offset().top - tagElem.height()/2);
});
$(".content_main").on("mouseover", ".PageView", function(evt) {
  // evt.stopPropagation();
  var tagElem = $(".tagger_content > div.newTagDraggable");
  if(tagElem.hasClass("NotarizeTabView")) {
    $(this).find(".pageHighlightOverlay").css({"background-color": hexToRgb(svgFillColor, 0.2), "border-color": svgFillColor});
  }
});
$(".content_main").on("mouseout", ".PageView", function(evt) {
  // evt.stopPropagation();
  $(this).find(".pageHighlightOverlay").css({"background-color": "transparent", "border-color": "transparent"});
});
...
function attachTagItem(target, posLeft, posTop) {
  var notaryTagPresent = false;
  var targetTagElement = $(".tagger_content > div.newTagDraggable");
  // console.log(targetTagElement);
  target.children(".NotarizeTabView").each(function() {
    if(targetTagElement.hasClass("NotarizeTabView") && ($(this).attr("data-tag-for") == targetTagElement.attr("data-tag-for"))) {
      notaryTagPresent = true;
    }
  });
  ...
  if(targetTagElement.length > 0) {
    ...
    if(!notaryTagPresent) {
      ...
      window.setTimeout(function(evt) {
        ...
        if(targetTagElement.hasClass("NotarizeTabView")) {
          var value_top = 0;
          target.find(".NotarizeTabView").each(function() {
            // target.css({"padding-top": parseInt(target.css("padding-top")) + 25});
            value_top += 25;
          });
          targetTagElement.css({"top": value_top, "left": "25px", "width": "calc(100% - 50px)", "height": "25px"});
          targetTagElement.find(".the_tag").css({"width": "100%", "background-color": hexToRgb(svgFillColor, 0.75)}).find(".tag_text").text("Notarize page");
          // target.find(".NotarizeTabView").css({"width": $(".PageView .pageHighlightOverlay").width()/(value_top/25)});
          // target.find(".NotarizeTabView .the_tag").css({"width": 100/(value_top/25) + "%"});
        }
      }, 100);
    } else {
      showToastMessageOlive($(".msg_notary_added"), "Page has already been marked for notarization");
      $(".tagger_content > div.newTagDraggable").remove();
    }
  }
...
$(".content_main").on("click", ".newTagDraggable.DropdownTabView .dropdown_menu .menu .item", function(evt) {
  ...
  $(this).closest(".menu").prev(".btn-trigger").find(".ng-binding.notary_dropdown_recipient .notary_recipient_name").text($(this).text().trim());
  $(this).closest(".menu").addClass("invisible");
});
...
$(".content_sidebar-left").on("click", ".sidebarTabs_tab", function(evt) {
  evt.stopPropagation();
  $(".sidebarTabs_tab").removeClass("sidebarTabs_tab-on").find(".icon").css("color", "#777");
  $(this).addClass("sidebarTabs_tab-on").find(".icon").css("color", svgFillColor);

  $(".sidebar-tab-option").addClass("hide");
  $(".sidebar-tab-option[data-tab-name='" + $(this).attr("data-qa") + "']").removeClass("hide");
});

$(".taggerDocument").on("mouseover", ".show_mini_tooltip", function(evt) {
  evt.stopPropagation();
  $(".tooltip-mini-notary").text($(this).attr("data-tooltip-text"));

  $(".tooltip-mini-notary").removeClass("invisible").css({"opacity": "1", top: $(this).offset().top + (($(this).height() - $(".tooltip-mini-notary").height())/2), left: $(this).offset().left + $(this).width()});
});
$(".taggerDocument").on("mouseout", ".show_mini_tooltip", function(evt) {
  evt.stopPropagation();
  $(".tooltip-mini-notary").addClass("invisible").css({"opacity": "0"});
});
