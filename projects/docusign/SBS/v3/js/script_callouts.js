...
$(document).ready(function(e) {
  setTimeout(function() {
    ...
    if(document.referrer == "http://www.protoux.us/s666/new_tagger.html" || document.referrer == "http://protoux.us/s666/new_tagger.html") {
      $("#referrer_if").html("another");
    } else {
      $("#referrer_if").html("a");
    }
...
$(".tooltip").on("click", ".btn-primary, .icon-times", function(e) {
  // console.log("btn");
  $(this).parents(".tooltip").addClass("hidden");
...
function showCallout(e, target) {
  ...
  if($(".tooltip").hasClass("before")) {
    ...
    $(".tooltip").css({left: $(target).offset().left - tooltipWidth});
  }
  if($(".tooltip").hasClass("after")) {
    $(".tooltip").css({left: $(target).offset().left + $(target).outerWidth() + 10});
  }
  if($(".tooltip").hasClass("left")) {
    $(".tooltip").css({left: $(target).offset().left + $(target).outerWidth()/2 - 30});
  ...
  if($(".tooltip").hasClass("right")) {
    $(".tooltip").css({left: $(target).offset().left + $(target).outerWidth()/2 - tooltipWidth + 40});
    // $(".tooltip").css({left: e.pageX - tooltipWidth + 40});
  }
  if($(".tooltip").hasClass("above")) {
    $(".tooltip").css({top: $(target).offset().top - tooltipHeight});
  }
  if($(".tooltip").hasClass("below")) {
    $(".tooltip").css({top: $(target).offset().top + $(target).outerHeight() + 10});
  }
  if($(".tooltip").hasClass("top")) {
    $(".tooltip").css({top: $(target).offset().top + $(target).outerHeight()/2});
