...
$(".recipient_cards").on("click", ".drawer_header .btn_save", function(e) {
  ...
  if(!error) {
    ...
    if(parent.hasClass("drawerAccess")) {
      if($(".tooltip.callout").hasClass("hidden") && validateAccessCode()) {
...
$(".recipient_cards").on("click", ".drawer_header .btn_cancel", function(e) {
  e.stopPropagation();
  var target = $(this);
  target.parents(".drawer").addClass("hide").css("height", 0);
  target.parents(".drawer").find("input.input-text:not(.disabled)").val("").removeClass("error_state");
  target.parents(".form_row").find(".AccessCode.text-alert").addClass("hidden");
  target.parents(".drawer").find("input.cb_input").attr("checked", false);
});
...
$(".recipient_cards").on("click", ".recipient_remove .icon-times", function() {
...
$(".recipient_cards").on("click", ".btn-group > .btn", function(e) {
  ...
  if($(this).attr("access-type") == "multiple") {
    ...
    targetEl.find(".selectTrigger").val("AccessCode").change();
  }
});

$("body").on("click", ".btn-trigger.action", function() {
  $("#menuOtherActions").toggleClass("invisible");
  $("#menuOtherActions").css({"top": $(this).position().top + $(this).outerHeight(), "left": $(this).offset().left - 20});
});
$("body").on("click", ".site-main", function() {
  $("#menuOtherActions").addClass("invisible");
});
$("body").on("click", ".edit_adv_options", function() {
  $(".modal").removeClass("hide");
  $("body").addClass("ds-modal-open");
});
$(".modal").on("click", ".close-x, .btn-minor, .btn-primary", function() {
  $(".modal").addClass("hide");
