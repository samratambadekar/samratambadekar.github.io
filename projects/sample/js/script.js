...
$(".editCallIcon, .editCallInfo, .deleteVMIcon, .voicemailTranscript, .selectItem").click(function(event){
...
function initChecks() {
	...
	$("#callBarringCheck2").prop("checked", false);
...
$(".voiceMailLine").click(function() {
	...
	/* MARK AS OPENED */
	if($(this).find(".voicemailTranscript").css("height") == "0px") {
		$(this).removeClass("isNotOpened");
	...
	if($(this).parents().hasClass("sequentialRingingSettings")) {
		if(parentEl.find(".addNumInput").val().trim() != "") {
			if($.isNumeric(parentEl.find(".addNumInput").val().trim())) {
				...
				parentEl.find(".numsList").append("<div><button class='removeNums hidden'><img src='images/arrowUp_black.png'></button><button class='removeNums hidden'><img src='images/arrowDown_black.png'></button><button class='removeNums hidden'>" + (parentEl.find(".numsList > div:not(.hidden)").length + 1) + "</button><span class='removeName'>(unknown)</span><span>" + parentEl.find(".addNumInput").val().trim() + "</span><img class='removeNums hidden' src='images/delete_cross.png'><img class='removeNums hidden' src='images/address.png'></div>");
			} else {
				...
				parentEl.find(".numsList").append("<div><button class='removeNums hidden'><img src='images/arrowUp_black.png'></button><button class='removeNums hidden'><img src='images/arrowDown_black.png'></button><button class='removeNums hidden'>" + (parentEl.find(".numsList > div:not(.hidden)").length + 1) + "</button><span class='removeName'>" + parentEl.find(".addNumInput").val().trim() + "</span><span>202-555-0160</span><img class='removeNums hidden' src='images/delete_cross.png'><img class='removeNums hidden' src='images/address.png'></div>");
			}
		...
		if(parentEl.find(".addNumInput").val().trim() != "") {
			if($.isNumeric(parentEl.find(".addNumInput").val().trim())) {
				parentEl.find(".numsList").append("<div><span>(unknown)</span><span>" + parentEl.find(".addNumInput").val().trim() + "</span><img class='removeNums hidden' src='images/delete_cross.png'><img class='removeNums hidden' src='images/address.png'></div>");
			} else {
				parentEl.find(".numsList").append("<div><span>" + parentEl.find(".addNumInput").val().trim() + "</span><span>202-555-0160</span><img class='removeNums hidden' src='images/delete_cross.png'><img class='removeNums hidden' src='images/address.png'></div>");
			}
			parentEl.find(".addNumsList").removeClass("hidden");
		}
	}	
...
$("#callBarringCheck2").click(function() {
	if($("#callBarringCheck2").prop("checked") == false) {
...
$("#callBarringSelect1").click(function() {
	if($("#callBarringSelect1").val() == "all") {
		$(this).parent().parent().find(".timePeriodSelector").addClass("hidden");
	} else if($("#callBarringSelect1").val() == "specific") {
...
$(".addEditContact .cancelButton").click(function() {
	var parentEl = $(this).parent().parent().css("height", 0);
});
$(".addEditContact").click(function(event) {
	event.stopPropagation();
});
